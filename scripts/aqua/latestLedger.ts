...
import { aquaRouter } from "./constants";
...
export async function getLatestRouterLedger(): Promise<any> {
  console.log("--------------------------------------------");
  console.log("Updating latest ledger for Aqua");
  console.log("--------------------------------------------");
  let latestLedger = 0;
  try {
    ...
    do {
      ...
      events = await toolkit.rpc.getEvents({
        ...
        filters: [
          {
            type: "contract",
            contractIds: [aquaRouter[process.env.NETWORK as NETWORK]],
            topics: [["AAAADwAAAARzd2Fw"]], // AquaRouter, swap in base64
          },
        ],
        limit: 100,
      });
