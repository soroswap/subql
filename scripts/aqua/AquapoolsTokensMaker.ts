...
async function getPoolReserves(
  poolAddress: string,
  poolType?: string
): Promise<{ reserveA?: string; reserveB?: string; hasLiquidity?: boolean }> {
  return retry(
    async () => {
      try {
    ...
    CONFIG.reserveRetryAttempts,
    CONFIG.reserveRetryDelay,
    CONFIG.retryBackoff
  );
}
...
async function processPoolWithReserves(
  index: number,
  totalSets: number
): Promise<AquaPool | null> {
  try {
    console.log(
      `🔍 Processing index ${index}/${totalSets - 1} (${(
        ((index + 1) / totalSets) *
        100
      ).toFixed(1)}%)`
    );
    ...
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, CONFIG.pauseBetweenReserveRequests)
      );
...
export async function getAquaPreStart(): Promise<void> {
  ...
  try {
    ...
    if (checkpoint) {
      console.log(
        `📝 Checkpoint found: index ${checkpoint.lastProcessedIndex}, ${checkpoint.poolsCount} pools`
      );
    ...
    for (let i = startIndex; i < totalSets; i += CONFIG.chunkSize) {
      ...
      console.log(
        `\n📦 Processing chunk ${
          Math.floor(i / CONFIG.chunkSize) + 1
        }/${Math.ceil(totalSets / CONFIG.chunkSize)} (indices ${i}-${
          i + chunk.length - 1
        })`
      );
      ...
      // Show progress
      console.log(
        `⏱️ Chunk time: ${(chunkTime / 1000).toFixed(
          2
        )}s | Total: ${formatElapsedTime(elapsedTotal)}`
      );
      console.log(`📈 Progress: ${progress}% | Remaining: ${timeRemaining}`);
      console.log(
        `📊 Pools: ${aquaPools.length} | Success: ${stats.successfulSets}/${stats.processedSets}`
      );

      // Pause between chunks (increased)
      await new Promise((resolve) =>
        setTimeout(resolve, CONFIG.pauseBetweenChunks)
      );
    }
...
`;
    ...
    console.log(
      `✅ Sets processed: ${stats.processedSets}/${stats.totalSets} (${(
        (stats.processedSets / stats.totalSets) *
        100
      ).toFixed(2)}%)`
    );
    console.log(
      `✅ Successful sets: ${stats.successfulSets} (${(
        (stats.successfulSets / stats.processedSets) *
        100
      ).toFixed(2)}%)`
    );
    ...
    console.log(
      `ℹ️ Pools with type: ${poolsWithType} (${(
        (poolsWithType / aquaPools.length) *
        100
      ).toFixed(2)}%)`
    );
    console.log(
      `💰 Pools with fee: ${poolsWithFee} (${(
